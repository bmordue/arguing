...
describe('Validation Strictness', () => {
    test('Node ID should not be a complex object', () => {
        const invalidGraph = {
            nodes: [
                { id: { complex: 'object' }, label: 'Invalid ID', type: 'node' }
            ],
            edges: []
        } as unknown as Graph;
    ...
    test('Node type should be a string', () => {
        const invalidGraph = {
            nodes: [
                { id: '1', label: 'Invalid Type', type: 123 }
            ],
            edges: []
        } as unknown as Graph;
    ...
    test('Edge label should be a string or array of strings', () => {
        const invalidGraph = {
            ...
            edges: [
                { source: '1', target: '2', label: 123 }
            ]
        } as unknown as Graph;

        expect(() => validateGraph(invalidGraph)).toThrow(/label must be a string or an array of strings/);
    });
    ...
    test('Edge label as array should only contain strings', () => {
        const invalidGraph = {
            ...
            edges: [
                { source: '1', target: '2', label: ['valid', 123] }
            ]
        } as unknown as Graph;
