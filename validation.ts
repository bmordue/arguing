...
export function validateGraph(graph: unknown): Graph {
    ...
    if (validatedGraph.nodes.length > MAX_NODES_COUNT) {
        throw new Error(`Graph nodes count exceeds maximum of ${MAX_NODES_COUNT}`);
    }
    if (validatedGraph.edges.length > MAX_EDGES_COUNT) {
        throw new Error(`Graph edges count exceeds maximum of ${MAX_EDGES_COUNT}`);
    }

    // Validate each node
    validatedGraph.nodes.forEach((node: Node, index: number) => {
        validateNode(node, index);
    });

    // Validate each edge
    validatedGraph.edges.forEach((edge: Edge, index: number) => {
